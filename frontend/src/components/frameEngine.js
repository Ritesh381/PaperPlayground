...
export function handleStageClick() {
    const frame = state.serverResponse.frames.find(f => f.id === state.currentFrameId);
    if (!frame) return;

    // If typing is in progress, skip it
    if (typingInterval && document.getElementById('vn-text').innerText.length < frame.text.length) {
        clearInterval(typingInterval);
